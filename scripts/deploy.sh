#!/usr/bin/env bash
# Deploys the Stello router to testnet and records what the clients need.
#
# Safe to re-run: identities and trustlines are only created when missing, and
# the contracts are redeployed with fresh ids every time (ticket and route
...
# Same values as a TypeScript module, so clients (Node and browser alike) can
# import them without JSON-module gymnastics.
{
  echo "// Generated by scripts/deploy.sh — do not edit."
  echo "export const deployment = $(cat deployments/testnet.json) as const;"
